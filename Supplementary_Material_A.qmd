---
title: "Evaluating the effects of tag mass on survival, breeding success, behaviour, and condition of a small, partially migratory shorebird"
...
packages_required_in_project <-
  c("apis", "BaSTA", "brms", "broom.mixed", "colorspace", "corrplot", "data.table", 
    ...
    "tidyterra", "tidyverse", "adehabitatLT", "magrittr", "MASS", "leaflet", "shiny", "leaflet.extras", "ggrepel", "ggpmisc", "grid")
...
```{r}
body_traits <- 
  read.csv(here("data/body_traits.csv"))
```
...
```{r}
cor.test(log(body_traits_avg$tarsus), log(body_traits_avg$weight)) # r = 0.127
...
body_traits_smi <-
  ...
  filter(!is.na(weight)) %>% 
  bind_cols(., smi_) %>% 
  dplyr::select(ring, tag_type, tag_type_, treatment_group, measure, SMI.ols, weight) %>% 
  na.omit() %>% 
...
body_traits_smi %>% 
  ...
  mutate(unit = ifelse(str_detect(statistic, "SMI"), "scaled mass index", "raw body mass"),
         attachment = ifelse(str_detect(statistic, "tag"), "only tag", "all attachments"),
...
```{r}
smi_wing_lmer_interaction <- 
  lmer(smi_wing ~ tag_type * measure + (1 | ring), 
       data = body_traits_smi, REML = FALSE)

smi_wing_lmer_noninteraction <- 
  lmer(smi_wing ~ tag_type + measure + (1 | ring), 
       data = body_traits_smi, REML = FALSE)

anova(smi_wing_lmer_interaction, smi_wing_lmer_noninteraction, test = "Chisq")
```
...
```{r, include=FALSE}
# saveRDS(stats_smi_wing_lmer, here("out/smi_results.rds"))
```

```{r, echo = FALSE}
stats_smi_wing_lmer <- readRDS(here("out/smi_results.rds"))
...
mod_comp_names <-
  data.frame(comp_name = c("Intercept (control group 1st capture)",
                           ...
                           "Control group 2nd capture",
                           "GPS group 2nd capture",
                           "PTT group 2nd capture",
...
fixef_bw_Table <-
  stats_smi_wing_lmer$partR2m$BW %>%
  as.data.frame() %>%
...
R2Table <-
  bind_rows(stats_smi_wing_lmer$partR2m$R2,
            stats_smi_wing_lmer$partR2c$R2[1,]) %>%
...
coefRptTable <-
  stats_smi_wing_lmer$rptR$R_boot %>%
  dplyr::select(-Fixed) %>%
...
```{r, include = FALSE}
# gtsave(smi_wing_lmer_table,
#        filename = here("tabs_figs/table_1.png"))
```
...
```{r, echo=FALSE}
# saveRDS(stats_breeding_brm, here("out/breed_results.rds"))
```
...
```{r, include=FALSE}
# saveRDS(surv_estimates, here("out/survival_results.rds"))
```
...
ggplot(behav %>% filter(!is.na(time)), aes(x = time)) +
  ...
  labs(title = "behavioural observations", x = "Time of Day", y = "Count") +
  theme_minimal()
...
### modelling
```{r, echo=FALSE}
inc_brood_model <- glmer(inc_brood ~ tagged + sin_time + cos_time + (1 | bird), data = behav, family = binomial)
walk_model <- glmer(walk ~ tagged + sin_time + cos_time + (1 | bird), data = behav, family = binomial)
feed_model <- glmer(feed ~ tagged + sin_time + cos_time + (1 | bird), data = behav, family = binomial)
fly_model <- glmer(fly ~ tagged + sin_time + cos_time + (1 | bird), data = behav, family = binomial)

summary(inc_brood_model)
summary(walk_model)
summary(feed_model)
summary(fly_model)
```
...
```{r}
# all_caps_KK %>%
#   filter(str_detect(code, "YR$"))# %>% 
#   # filter(code %in% c("XX.RW|MX.BG", "XX.RW|MX.GB"))
# 
# all_caps_KK %>% 
#   filter(tag_type %in% c("GPS", "PTT")) %>% 
#   group_by(tag_type) %>% 
#   summarise(n_birds = n_distinct(code))
# 
# all_caps_KK %>% 
#   mutate(bird = ifelse(nchar(code) == 12, paste0(str_sub(code, 5, 6), str_sub(code, 11, 12)),
#                        ifelse(nchar(code) == 11, paste0(str_sub(code, 4, 5), str_sub(code, 10, 11)), 
#                               paste0(str_sub(code, 5, 6), str_sub(code, 12, 13))))) %>% 
#   select(bird, tag_type, treatment_group, tag_id) %>% 
#   group_by(bird) %>% 
#   arrange(tag_type) %>% 
#   distinct() %>%
#   slice_tail(n = 1) %>% group_by(tag_type) %>% summarise(n_birds = n_distinct(bird))
# 
# bdot_tagging_history <- 
#   read.csv("/Users/luketheduke2/ownCloud/kemp_projects/bdot/R_projects/bdot_ethics/data/mark_recapture_df.csv") %>% 
#   rename(parent = ID) %>% 
...
#   mutate(deployment_date = ifelse(!is.na(deployment_date), deployment_date, NA),
#          recapture_date = ifelse(!is.na(recapture_date), recapture_date, NA)) %>% 
...
#   arrange(tag_type) %>% 
#   filter(!is.na(tag_type))
#   
# winter_2021 <- 
#   read_excel("/Users/luketheduke2/ownCloud/kemp_projects/bdot/data/from_Ailsa/Sightings Banded birds Final 221206.xlsx",
#              sheet = "Winter 2021", skip = 4, col_names = FALSE,
#              col_types = "text") %>% 
#   rename(date = `...1`,
#          bird = `...2`,
#          location = `...3`,
...
#   mutate(bird = str_replace_all(bird, "\\?", "")) %>%
#   filter(str_length(bird) == 4) %>% 
#   left_join(., tag_date_info %>% rename(bird = parent) %>% select(bird, tag_type, deployment_date, recapture_date)) %>% 
#   mutate(tagged_status = ifelse(deployment_date < date & recapture_date > date & !is.na(deployment_date) & !is.na(recapture_date), 1, 0)) %>% 
#   filter(month(date) %in% c(3, 4, 5, 6))#%>% filter(is.na(tag_type)) %>% pull(bird) %>% unique() %>% length()
# 
# winter_2022 <-
#   read_excel("/Users/luketheduke2/ownCloud/kemp_projects/bdot/data/from_Ailsa/Sightings Banded birds Final 221206.xlsx",
#              sheet = "202122",
...
# 
# bind_rows(winter_2021, winter_2022) %>% 
#   mutate(year_ = year(date),
#          tag_type = ifelse(is.na(tag_type), "C", tag_type)) %>% 
#   group_by(tag_type) %>% 
#   summarise(n_birds = n_distinct(bird))
```
...
#### summary of deployment duration (days)
```{r}
tag_metadata %>% 
  filter(tag_type != "control") %>% 
  group_by(tag_type) %>% 
  summarise(mean_deploy_duration = mean(deploy_duration, na.rm = TRUE),
            max_deploy_duration = max(deploy_duration, na.rm = TRUE),
            min_deploy_duration = min(deploy_duration, na.rm = TRUE))
```
...
#### summary of operational duration (days)
```{r}
move %>% 
  group_by(parent, tag_type) %>% 
  summarise(total_fixes = n(),
            max_time = max(time_since_deployment, na.rm = TRUE)) %>% 
  group_by(tag_type) %>% 
  summarise(mean_deployment_duration = mean(max_time)/86400,
            sd_deployment_duration = sd(max_time)/86400,
            min_deployment_duration = min(max_time)/86400,
            max_deployment_duration = max(max_time)/86400)
```
...
### interactive map of all tagging data
to view, select the birds you want to see and the date range then click "Update Map"
```{r, eval=FALSE}
move_sf <- 
  move %>% 
  ...
  rename(bird = parent) %>% 
  mutate(bird = paste0(str_sub(bird, 1, 2) , "-", str_sub(bird, 3, 4), " / ", tag_type))
...
color_pal <- colorFactor(rainbow(length(birds)), birds)

# Define UI
ui <- fluidPage(
  titlePanel("Kaikōura Banded Dotterel Tracking Map"),
  sidebarLayout(
    sidebarPanel(
      checkboxGroupInput("bird", "Select bird(s):", choices = birds, selected = birds),
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    req(input$bird, input$dateRange)
    data %>%
      filter(bird %in% input$bird,
  ...
  observeEvent(input$select_all, {
    updateCheckboxGroupInput(session, "bird", selected = birds)
  })
  
  # Observer for "Deselect All" button
  observeEvent(input$deselect_all, {
    updateCheckboxGroupInput(session, "bird", selected = character(0))
  })
  ...
  observeEvent(input$update, {
    ...
    if (nrow(data) == 0) {
      ...
      # Split data by bird
      data_split <- data %>% group_split(bird)
      
      # Add paths grouped by bird
      for (group in data_split) {
        coords <- st_coordinates(group)
        leafletProxy("map") %>%
          addPolylines(lng = coords[, 1], lat = coords[, 2], color = color_pal(group$bird[1]), group = group$bird[1])
      }
      
      # Extract coordinates for markers and create popups
      data_coords <- st_coordinates(data)
      popups <- paste("<strong>Bird / Tag Type:</strong>", data$bird, "<br>",
                      "<strong>Date:</strong>", data$date_local)
      
      # Compute colors for each marker
      marker_colors <- color_pal(data$bird)
...
max_dist_location <-
  ...
  st_drop_geometry() %>%
  filter(locationClass %in% c(NA, 1, 2, 3)) %>% 
  dplyr::select(parent, tagID, loc_longitude, loc_latitude, date_local) %>% 
  # filter(loc_longitude > 0 & loc_latitude < 0) %>% 
  ...
  mutate(loc_longitude = ifelse(parent == "RBRL", RBRL_mokau_lon, loc_longitude),
...
# au_water <-
#   st_read("/Users/luketheduke2/Documents/Academic/New_Zealand/banded_dotterels/data/spatial/NZ_map/AUS_wat/AUS_water_areas_dcw.shp") %>%
#   # st_transform(lcc_params) %>% 
#   filter(HYC_DESCRI == "Perennial/Permanent")

nz_water <-
  st_read("/Users/luketheduke2/Documents/Academic/New_Zealand/banded_dotterels/data/spatial/NZ_map/NZL_wat/NZL_water_areas_dcw.shp") %>%
  # st_transform(lcc_params) %>% 
  filter(HYC_DESCRI == "Perennial/Permanent")
...
# plot(nz_dem_proj)
# nz_au_dem <- raster::raster("/Users/luketheduke2/Documents/Academic/New_Zealand/banded_dotterels/data/spatial/NZ_map/DEM_zoom_6.tif")
...
parent_groups <- 
  BADO_curve_plot_df %>% group_by(parent) %>% mutate(n_points = n()) %>% filter(n_points > 1)
...
```{r}
# m <- ggplot() +
#   # Base map with NZ boundaries
#   geom_sf(data = nz_bound_crop, fill = "white", color = "white") +
#   
#   # Hillshade raster overlay
#   geom_raster(data = hillmultidf, aes(x, y, fill = sum), alpha = 0.5, show.legend = FALSE) +
#   scale_fill_distiller(palette = "Greys") +
#   
#   # Elevation raster overlay
#   new_scale_fill() +
#   geom_raster(data = mdtdf, aes(x, y, fill = alt), alpha = 0.5) +
#   scale_fill_gradientn(
#     colors = nz_pal$hex,
#     values = scales::rescale(nz_pal$limit),
#     limits = range(nz_pal$limit)
#   ) +
#   
#   # Blending layers
#   blend("multiply") +
...
#   geom_point(data = BADO_curve_plot_df %>% 
#                filter(parent %in% c("RWLG", "RBBR", "RRRG", "RBOY")) %>% 
#                group_by(parent) %>% arrange(date_local) %>% slice(2),
...
#   
#   geom_point(data = Kaikoura, 
#              aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]), 
#              size = 5, shape = 21, fill = "grey40", stroke = 0.8, color = "grey40") +
#   
#   # Track paths
#   geom_path(data = RBRL_track, 
#             aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]), 
#             linewidth = 0.5, linetype = "dashed", color = "grey40") +
#   
#   geom_point(data = RBRL_track %>% slice(n()),
#              aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]), 
#              size = 3, shape = 21, fill = "grey40", stroke = 0.8, color = "grey40") +
#   
#   # Guides and labels
#   guides(fill = guide_colorsteps(barwidth = 20, barheight = 0.5, title.position = "right")) +
#   labs(fill = "m", x = "Longitude", y = "Latitude") +
#   
#   # Coordinate system
#   coord_sf(
#     xlim = c(bbox["xmin"] - 650000, bbox["xmax"] + 450000),
