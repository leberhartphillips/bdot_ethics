...
```{r, echo=FALSE}
anova_results <- anova(smi_wing_lmer_interaction, smi_wing_lmer_noninteraction, test = "Chisq") %>%
  broom::tidy()
...
tag_metadata %>% 
  ...
  group_by(tag_type) %>% 
  summarise(mean = mean(deploy_duration, na.rm = TRUE),
            max = max(deploy_duration, na.rm = TRUE),
            min = min(deploy_duration, na.rm = TRUE)) %>%
  kable(format = "html", caption = "summary of deployment duration (days)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = FALSE)
```
...
move %>% 
  ...
  summarise(total_fixes = n(),
            max_time = max(time_since_deployment, na.rm = TRUE)) %>% 
  group_by(tag_type) %>% 
  summarise(mean = mean(max_time)/86400,
            sd = sd(max_time)/86400,
            min = min(max_time)/86400,
            max = max(max_time)/86400) %>%
  kable(format = "html", caption = "summary of operational duration (days)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = FALSE)
```
